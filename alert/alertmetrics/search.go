...
func (s *Store) Search(ctx context.Context, opts *SearchOptions) ([]AlertDataPoint, error) {
	...
	metrics := make([]AlertDataPoint, 0)

	for rows.Next() {
		var dp AlertDataPoint
		err = errors.Wrap(dp.scanFrom(rows.Scan), "query")
		if err != nil {
			return nil, err
		}
