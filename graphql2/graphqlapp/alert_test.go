...
func TestSplitRangeByDurationAlertCounts(t *testing.T) {
	...
	check := func(desc string, r timeutil.ISORInterval, data []alertmetrics.AlertDataPoint, exp []int) {
		t.Helper()
		actual := []int{}
	...
	// ...
	jan := []alertmetrics.AlertDataPoint{}
	for day := 0; day < 20; day++ {
		for hour := 0; hour <= day; hour++ {
			jan = append(jan, alertmetrics.AlertDataPoint{
